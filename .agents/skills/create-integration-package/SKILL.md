...
At a minimum, test the following two combinations.
 ...
 -  `mise test:init -w framework`: Tests all package manager, KV store,
    ...
    required database is not installed or running, its combinations are
    skipped, so additional installation and startup may be needed.  If you
    are an agent, ask the user whether to install and run it before
    proceeding.  If the user declines, ignore the skipped combinations.

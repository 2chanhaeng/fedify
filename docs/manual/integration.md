...
[Nuxt] is a full-stack web framework built on top of [Vue] and [Nitro]. The
_@fedify/nuxt_ package provides a server middleware to integrate Fedify with
Nuxt:
...
First, create a server middleware in _server/middleware/federation.ts_:
...
export default fedifyMiddleware(federation, (_event) => undefined); // [!code highlight]
```

Then, create a Nitro error handler in _server/error.ts_ to handle
[content negotiation] when a Fedify route falls back to the app's own 404
handler:

```typescript
export { onError as default } from "@fedify/nuxt"; // [!code highlight]
```

Finally, configure the `nitro.errorHandler` option in _nuxt.config.ts_ to point
to this file:

...
| `fedify.queue.role`                          | string   | The Fedify queue role: `inbox`, `outbox`, `fanout`, or `task`; `shared` additionally appears on queue depth rows where one queue backs multiple roles.                                                                | `"outbox"`                                                           |
